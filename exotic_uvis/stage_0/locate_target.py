...
def locate_target(direct_image):
    '''
    Opens the direct image and finds the target
    ...
    satisfied = False       # whether the user is happy with the location identifiede by DAOStarFinder
    dist_threshold = 100    # how far to search from the initial guess
    ...
    with fits.open(direct_image) as fits_file:
        ...
        while not satisfied:
            ...
            for ind1, ind2 in zip(sources[1][:], sources[2][:]):
                if (((ind1-bestx)**2 + (ind2-besty)**2)**0.5 < dist_threshold):
            ...
            check = int(input("Enter 0 to keep this source, 1 to search again,\n2 to update the threshold and then search again, or 3 to select manually: "))
            if check == 0:
                satisfied = True
            if check == 2:
            ...
            if check == 3:
                ...
                plt.subplot(1,1,1)
                im = plt.imshow(np.log10(np.abs(d)), origin="lower", cmap="binary_r")
                plt.scatter(xs, ys, s=8, marker="x", color="red")
                plt.xlim(int(xs-70), int(xs+70))
                plt.ylim(int(ys-70), int(ys+70))
                plt.show()
