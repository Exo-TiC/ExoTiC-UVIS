...
def time_and_relative_detrending_in_space(obs, trace_x, trace_y,
                                          wav, order,
                                          sigma=3.5, flux_threshold=1000, 
                                          window=10, replacement='tseries',
                                          verbose = 0, show_plots = 0, save_plots = 0, output_dir = None):
    """Estimates systematic trends to catch cosmic rays otherwise not detected
    by any of the Stage 1 cleaning routines.
    ...
    Args:
        ...
        wav (array-like): wavelength solution attached th trace_x. Used to
        adjust window to be shorter at UV wavelengths.
    ...
    # Map window size to wavelength, decreasing window size by 70% at UV edge.
    windows = np.array([round(x) for x in np.linspace(window,window*0.3,len(trace_x))])
    if wav[0] < wav[-1]:
        # Reverse windows.
        windows = np.array([round(x) for x in np.linspace(window*0.3,window,len(trace_x))])

    # Plot the windows if asked.
    if save_plots > 0 or show_plots > 0:
        plt.figure(figsize=(5,7))
        plt.scatter(wav,windows,color='k',marker='o')
        plt.plot(wav,windows,ls='--',color='k')
        plt.xlabel(r'Wavelength ($\AA$)')
        plt.ylabel('Window Halfwidth [pixels]')
        plt.title('Window Sizes for TARDIS')

        if save_plots > 0:
            plot_dir = os.path.join(output_dir, 'plots')
            if not os.path.exists(plot_dir):
                os.makedirs(plot_dir) 
            filedir = os.path.join(plot_dir, f'tardis_windows_order{order}.png')
    ...
    xl,xu,yl,yu = (np.min(trace_y)-30,np.max(trace_y)+30,
                   np.min(trace_x)-5,np.max(trace_x)+5)
    ...
    for x, y in tqdm(zip(xfix,yfix),total=len(xfix),
                     desc='Identifying CRs in trace region... Progress:',
                     disable=(verbose < 1)):    
        # Update window based on x.
        index = np.argmin(np.abs(x-trace_x))
        ...
        hit_map[:,x,y] = np.where(detrended_series-med>sigma*sig,1,hit_map[:,x,y])
