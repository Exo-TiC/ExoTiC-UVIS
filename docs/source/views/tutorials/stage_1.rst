...
.. code-block:: bash
  ...
  do_hst_flags    True                                        # Bool. If True, reads in HST data quality information and corrects for chosen HST flags.
  ...
  include_hst_dq  True                                        # Bool. If True, includes HST data quality flag information in the quicklook gif.
...
Step 2: Correct pixels flagged by HST pipeline
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The HST :code:`calwf3` pipeline flags pixels for various systematic faults (e.g. saturated, hot, dead, etc.) with its own data quality flags, which are documented in the `wfc3tools <https://wfc3tools.readthedocs.io/en/latest/wfc3tools/calwf3.html#data-quality-array-initialization-dqicorr>`_. Set :code:`do_hst_flags` to True if you want to correct pixels flagged by :code:`calwf3`. Use the :code:`hst_flags` list variable to select which flags you want to correct. Set :code:`hst_replace` to True to replace bad values with spatial or temporal medians, or set it False to simply record HST flag information within the data quality array.
...
Step 6c: Background star tracking
'''''''''''''''''''''''''''''''''
If there are unsaturated background stars available in your frame, they may be more useful to track than the 0th order. Set :code:`do_bkg_stars` to True to run this routine. For each background star you wish to track, supply its x and y coordinates as a list entry in the :code:`bkg_stars_loc` list. If your star is near another bright source that may affect tracking if included within the tracking window, use the :code:`bkg_window` parameter to shrink the halfwidth of the window in which the star is tracked.
  
Step 7: Quality quicklook
~~~~~~~~~~~~~~~~~~~~~~~~~
If you want to save a "quicklook" gif that presents all of the data frames in succession as well as simple diagnostics of the image and trace flux, simply set :code:`do_quicklook` to True. You can use the :code:`traces_included` variable to include flux from the +1 trace, the -1 trace, or both. Additionally, this will save a quicklook gif made using the data quality array, which can be checked to ensure that your reduction has not overcorrected the data. Set :code:`include_hst_dq` to True to include HST :code:`calwf3` data quality in this gif, or set it False to only see what :code:`HUSTLE-tools` flagged.
